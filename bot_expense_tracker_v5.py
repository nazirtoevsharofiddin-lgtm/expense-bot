...
DEBT_OPEN_TAG = "#debt_open"
DEBT_CLOSED_TAG = "#debt_closed"
...
def parse_date_or_today(date_str: Optional[str]) -> str:
    """Валидирует дату в формате DD.MM.YYYY, иначе возвращает сегодня."""
    if date_str:
        try:
...
def is_debt_category(category: str) -> bool:
    return category == "Долг"
...
- "Дал в долг" (я даю деньги в долг): "Долг" > "Дал в долг". Примеры: "дал в долг 100к", "одолжил Алишеру 200000"
- "Мне вернули долг" (кто-то вернул деньги): "Долг" > "Мне вернули долг". Примеры: "вернули 100к", "Алишер вернул долг"
- "Взял в долг" (я беру деньги в долг): "Долг" > "Взял в долг". Примеры: "взял долг 500000", "занял у брата 200000", "одолжил у соседа"
- "Отдал долг" (я возвращаю деньги): "Долг" > "Отдал долг". Примеры: "отдал долг 100к", "вернул брату 200000"
...
{{"amount": 50000, "description": "кофе", "category": "Еда и напитки", "subcategory": "Рестораны и кафе", "is_income": false, "is_debt": false, "date": null}}"""
    ...
    def write_expense(self, amount: float, description: str, category: str,
        ...
        expense_id = f"{record_date}_{time_str}_{amount}".replace(".", "").replace(":", "")
    ...
    full_comment = (full_comment + " " + DEBT_OPEN_TAG).strip()
        ...
        ws = self.sheet.worksheet("Расходы")
        ws.append_row([record_date, time_str, description, subcategory, category, stored_amount, full_comment, expense_id])
        ...
        header = "Долг (не влияет на бюджет)" if is_debt else f"{literacy_group}  ›  {category}"
        msg = (
            f"{color} <b>{header}</b>\n"
    ...
    def settle_debt(self, amount: float, description: str = "") -> Optional[Tuple[int, str, float]]:
        """
        Находит наиболее подходящий непогашенный долг и закрывает его
        (помечает как закрытый, деньги просто возвращаются в баланс,
        без учёта как новый доход).
        Возвращает (row_idx, description, amount) закрытого долга либо None.
        """
    ...
    def settle_payable(self, amount: float, description: str = "") -> Optional[Tuple[int, str, float]]:
        """Закрывает долг, который я должен (payable)."""
    ...
    def get_literacy_stats(self, days: int = 30) -> Tuple[str, InlineKeyboardMarkup]:
        ...
        balance = total_income - total_expenses + debt_net
        sign = "+" if balance >= 0 else ""
        msg += f"💰 Баланс: {sign}{format_amount(balance)} сум\n"
        if debt_net != 0:
            dsign = "+" if debt_net >= 0 else ""
            msg += f"🤝 <i>из них долги (не влияют на бюджет): {dsign}{format_amount(debt_net)} сум</i>\n"
        msg += "\n"
    ...
    def get_category_stats(self, days: int = 30) -> Tuple[str, InlineKeyboardMarkup]:
        ...
        for row in rows[1:]:
            ...
            try:
                ...
                if is_debt_category(cat):
                    continue  # долги не входят в статистику по категориям
    ...
    def get_top_expenses(self, limit: int = 5) -> str:
        ...
        for row in rows[1:]:
            ...
            try:
                ...
                amount = float(row[5])
                if amount < 0:
                    expenses.append((row[2], cat, abs(amount)))
            except:
                pass
    ...
    def get_month_summary(self) -> Tuple[str, Optional[InlineKeyboardMarkup]]:
        ...
        cats_with_data = sorted(
            [(cat, expenses_sum.get(cat, 0)) for cat in EXPENSE_CATEGORIES if cat not in ("Доход", "Долг")],
    ...
    def get_quick_stats(self) -> str:
        ...
        td, wd, md = 0.0, 0.0, 0.0  # debt_net за периоды
        for row in rows[1:]:
            if len(row) < 6:
                continue
            try:
                ...
                cat = row[4]
                amount = float(row[5])
                debt = is_debt_category(cat)
                if date_str == today:
                    if debt: td += amount
                    elif amount > 0: ti += amount
                    else: te += abs(amount)
                if exp_date >= week_ago:
                    if debt: wd += amount
                    elif amount > 0: wi += amount
                    else: we += abs(amount)
                if exp_date >= month_ago:
                    if debt: md += amount
                    elif amount > 0: mi += amount
                    else: me += abs(amount)
            except:
                pass
        return (
            f"⚡ <b>Быстрая статистика</b>\n\n"
            f"📅 <b>Сегодня:</b> +{format_amount(ti)} / -{format_amount(te)} = {format_amount(ti-te+td)}\n"
            f"📆 <b>Неделя:</b>  +{format_amount(wi)} / -{format_amount(we)} = {format_amount(wi-we+wd)}\n"
            f"📊 <b>Месяц:</b>   +{format_amount(mi)} / -{format_amount(me)} = {format_amount(mi-me+md)}"
        )
    ...
    def get_all_time_balance(self) -> str:
        """All-time баланс: доход, расходы, долги."""
        ws = self.sheet.worksheet("Расходы")
        rows = ws.get_all_values()
        total_income = 0.0
        total_expenses = 0.0
        ...
        msg += f"💚 Всего дохода: +{format_amount(total_income)} сум\n"
        msg += f"📉 Всего расходов: -{format_amount(total_expenses)} сум\n"
        balance_sign = "+" if all_time_balance >= 0 else ""
        msg += f"💰 Баланс: {balance_sign}{format_amount(all_time_balance)} сум\n\n"
        msg += f"🤝 Вам должны: {format_amount(receivables_total)} сум\n"
        msg += f"💳 Вы должны: {format_amount(payables_total)} сум"
        return msg
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "• зарплата 8 500 000\n"
        "• эхсон 100 000\n"
        "• 15 июля такси 30000 (задним числом)\n\n"
        "🤝 <b>Долги:</b>\n"
        "• дал в долг Алишеру 200000\n"
        "• вернул долг Алишер 200000 (закроется автоматически)\n\n"
        ...
        "/debts — непогашенные долги\n"
        "/undo — удалить последние записи\n"
        "/undoall — удалить из ВСЕХ записей\n"
        "/export — скачать CSV\n"
        "/help — справка"
    )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    category = result.get('category', 'Покупки')
    subcategory = result.get('subcategory', '')
    is_income = result.get('is_income', False)
    is_debt = result.get('is_debt', False)
    date = result.get('date')

    # Автозакрытие долга: мне вернули долг, который я давал
    if category == "Долг" and subcategory == "Получить долг":
        settled = tracker.settle_debt(amount, description)
        msg = tracker.write_expense(
            amount, description, category, subcategory,
            "", True, date, True
        )
        if settled:
            _, closed_desc, closed_amount = settled
            msg += f"\n\n✅ <i>Закрыт долг: {closed_desc} ({format_amount(closed_amount)} сум)</i>"
        else:
            msg += "\n\n⚠️ <i>Не нашёл открытый долг для закрытия — добавил как есть</i>"
        await update.message.reply_text(f"✅\n\n{msg}", parse_mode=ParseMode.HTML)
        return

    msg = tracker.write_expense(
        amount, description, category, subcategory,
        "", is_income, date, is_debt
    )
    await update.message.reply_text(f"✅\n\n{msg}", parse_mode=ParseMode.HTML)
...
async def debts_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    tracker = context.bot_data.get('tracker')
    outstanding = tracker.get_outstanding_debts()
    if not outstanding:
        await update.message.reply_text("🤝 Нет непогашенных долгов")
        return
    msg = "🤝 <b>Непогашенные долги (вам должны вернуть):</b>\n\n"
    total = 0.0
    for idx, (row_idx, date_str, desc, amount, comment) in enumerate(outstanding, 1):
        msg += f"{idx}. {format_amount(amount)} сум — {desc} ({date_str})\n"
        total += amount
    msg += f"\n<b>Итого вам должны:</b> {format_amount(total)} сум"
    await update.message.reply_text(msg, parse_mode=ParseMode.HTML)
...
# --- Удаление ---
async def _show_delete_list(update: Update, context: ContextTypes.DEFAULT_TYPE, limit: int):
    tracker = context.bot_data.get('tracker')
    expenses = tracker.get_expenses(days=3650, limit=limit)
    if not expenses:
        await update.message.reply_text("❌ Нет записей")
        return ConversationHandler.END
    msg = "📋 <b>Какую запись удалить?</b>\n\n"
    ...
    if len(args) > 1:
        if args[1] == "all":
            ...
            try:
                ...
                limit = 5
    return await _show_delete_list(update, context, limit)


async def undoall_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Шорткат: показывает ВСЕ записи для удаления (аналог /undo all)."""
    return await _show_delete_list(update, context, 100000)
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "<b>Долги (🤝) не входят в бюджет:</b>\n"
        "Когда даёшь в долг — не расход. Когда возвращают — не доход.\n"
        "Просто движение денег в кошельке. /debts — список непогашенных.\n\n"
        "<b>Задняя дата:</b>\n"
        "Пиши дату прямо в сообщении: «15 июля кофе 20000», «вчера такси 30000»\n\n"
        "<b>Статистика:</b>\n"
...
def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            ...
            CommandHandler("undoall", undoall_cmd),
        ],
        states={
