...
# -*- coding: utf-8 -*-
"""
Telegram бот для трекинга расходов v4
С поддержкой доходов
"""
...
KEYWORDS = {
    "Быт": {
        "еда": ["кофе", "чай", "есть", "обед", "завтрак", "ужин", "сок", "фастфуд"],
        "покупки": ["платье", "обувь", "рубашка", "шапка", "купил", "покупка", "магазин"],
        "жилье": ["квартира", "аренда", "ипотека", "коммунал", "электричество", "газ", "вода"],
        "транспорт": ["такси", "автобус", "метро", "поезд", "билет", "бензин", "парковка"],
        "связь": ["интернет", "телефон", "мобильный", "оператор"],
        "развлечение": ["кино", "концерт", "игра", "спорт", "тренировка", "подписка", "хобби"],
    },
    "Образование": {
        "книги": ["книга", "ebook", "аудио"],
        "развитие": ["курс", "школа", "лекция", "мастер", "обучение"],
        "подписки": ["подписка", "newsletter", "канал"],
    },
    "Инвестиции": {
        "финансы": ["акции", "облигации", "крипто", "инвестиция", "инвест"],
        "сбережения": ["сбережения", "депозит", "вклад", "золото"],
    },
    "Reciprocity": {
        "подарок": ["подарок", "подарил", "дал", "пожертвовал"],
        "помощь": ["помощь", "помог", "благотворительность", "черити", "дарение"],
    },
    "Долги": {
        "долг": ["долг", "заём", "кредит"],
        "платеж": ["платёж", "комиссия", "сбор", "штраф", "налог"],
    },
    "Доход": {
        "зарплата": ["зарплата", "оклад", "зп", "зарплату"],
        "доход": ["доход", "заработал", "получил", "поступило", "гонорар"],
        "премия": ["премия", "бонус", "награда", "подарок деньги"],
    },
}
...
def normalize_number(num_str: str) -> float:
    """Нормализует число из разных форматов: 5000, 5 000, 5.000, 5,000"""
    ...
    if "." in num_str or "," in num_str:
        # Проверяем контекст: если разделитель в конце на 2 знака (5.00 или 5,00)
        if re.search(r'[.,]\d{2}$', num_str):
            # Это децимальная часть, заменяем запятую на точку
            num_str = num_str.replace(",", ".")
        else:
            # Это разделитель тысяч, удаляем
...
class ExpenseTracker:
    ...
    def parse_expense(self, text: str) -> Optional[Dict]:
        """Парсит расход или доход с поддержкой комментариев"""
        text = text.lower().strip()
        
        # Ищем число (может быть с разными форматами)
        match = re.search(r'[-]?[\d\s.,_]+', text)
        if not match:
            return None
        ...
        # Остаток текста — описание и возможный комментарий
        remainder = re.sub(r'[-]?[\d\s.,_]+', '', text).strip()
        
        # Разделяем на описание и комментарий (по " - ")
        if " - " in remainder:
            description, comment = remainder.split(" - ", 1)
        else:
        ...
        category = "Быт"
        subcategory = "Жизнь и развлечения"
        
        # Сначала проверяем доход
        if "Доход" in KEYWORDS:
            for subcat, keywords in KEYWORDS["Доход"].items():
                for keyword in keywords:
                    if keyword in text:
                        ...
                        if subcat == "зарплата":
                            ...
                            subcategory = "Премия"
                        return {
                            "amount": abs(amount),  # Доход всегда положительный
            ...
            for subcat, keywords in subcats.items():
                for keyword in keywords:
                    if keyword in text:
                        ...
                        if cat == "Быт":
                            if subcat == "еда":
                                subcategory = "Еда и напитки"
                            elif subcat == "покупки":
                                subcategory = "Покупки"
                            elif subcat == "жилье":
                                subcategory = "Жилье"
                            elif subcat == "транспорт":
                            ...
                            subcategory = "Образование и развитие" if subcat != "книги" else "Книги"
                        elif cat == "Инвестиции":
                            subcategory = "Финансовые инвестиции" if subcat == "финансы" else "Сбережения"
                        elif cat == "Reciprocity":
                            subcategory = "Подарки" if subcat == "подарок" else "Благотворительность"
                        elif cat == "Долги":
                            subcategory = "Долги"
                        return {
                            "amount": abs(amount),  # Расходы всегда положительные в сумме
                            "description": description.strip(),
                            "category": category,
                            "subcategory": subcategory,
                            "comment": comment.strip(),
    ...
    def write_expense(self, amount: float, description: str, category: str, subcategory: str, comment: str = "", is_income: bool = False) -> str:
        ...
        # Для дохода сумма положительная, для расходов отрицательная в базе
    ...
    def get_top_expenses(self, limit: int = 5) -> str:
        ...
        for row in rows[1:]:
            ...
            try:
                amount = float(row[5])
                if amount < 0:  # Только расходы (отрицательные)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /start"""
    text = (
        "👋 <b>Привет! Я трекер расходов</b>\n\n"
        "📝 <b>Как использовать:</b>\n"
        "<b>Расходы:</b>\n"
        "• кофе 50000\n"
        "• книга 85000 образование\n"
        "• 8500000 - встреча\n\n"
        "<b>Доходы:</b>\n"
        "• зарплата 8500000\n"
        "• доход 500000\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = tracker.parse_expense(text)
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "<b>Ключевые слова дохода:</b>\n"
        "зарплата, оклад, доход, получил, премия, бонус\n\n"
        "<b>Форматы чисел:</b>\n"
        "5000, 5 000, 5.000, 5,000 — все работают\n\n"
        "<b>Комментарии:</b>\n"
        "кофе 50000 - встреча с клиентом"
    )
    await update.message.reply_text(text, parse_mode=ParseMode.HTML)
